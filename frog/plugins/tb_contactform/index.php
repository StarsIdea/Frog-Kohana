...
function TB_Contactform($emailTo = "", $emailCC = FALSE, $sentHeading='Your message was sent successfully', $sentMessage='Please allow up to three days for a reply.') {
	...
	if(isset($_POST['submit']))
	{
		...
		$recipient  = makeSafe($_POST['recipient']);
		...
		else
		{
			...
			switch ($recipient):
				case 0:
					$emailTo = "membership@enduranceridersofalberta.com";
					break;
				case 1:
					$emailTo = "president@enduranceridersofalberta.com";
					break;
				case 2:
					$emailTo = "membership@enduranceridersofalberta.com";
					break;
				default:
					$emailTo = "christy@christinaweese.net";
					break;
			endswitch;			
		
			$content = $name . ' (' . $email . ') wrote the following at ' . date("g:ia \o\\n l, F jS") . ":\n\n";
			$content .= $message ."\n\n";
...
function TB_Displayform($error = false) {
	...
	<select id="recipient" name="recipient">
	  ...
	  <option value="1">ERA President</option>
	  <option value="2">Membership Chair</option>
	</select>
	</p>
	';
	...
	echo '	<p class="submit">'."\n";
	echo '		<input type="submit" id="tbsubmit" class="submit" name="submit" value="Send Email" />'."\n";
	echo '	</p>'."\n";
	echo '	</form>'."\n".'</div>'."\n";
}
...
{
	return trim(addslashes(htmlentities(htmlspecialchars(strip_tags($data)))));
}
