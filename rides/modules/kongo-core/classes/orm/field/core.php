...
/**
 * Basic ORM Field Implementation.
 * Based off of Woody Gilk's Sprig_Field_Core.
 *
 * @package	   Kongo-Core
 * @author	   Ryan Padget
 * @copyright  (c) 2011 Ryan Padget
 * @license	   MIT
 */
abstract class ORM_Field_Core {
	...
	public function __construct(array $options = NULL)
	{
		if ( ! empty($options))
		{
			$this->null = $options['is_nullable'];
			$this->max_length = Arr::get($options, 'character_maximum_length');
			if( ! $this->max_length) {
				$this->max_length = Arr::get($options, 'max');
			}
